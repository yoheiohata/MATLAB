...
x = [notch_f1 notch_f1];
y = [y_low y_high];
line(x,y,'Color','green','linestyle','--',"DisplayName","f(noise)="+notch_f1);
x = [notch_f2 notch_f2];
y = [y_low y_high];
line(x,y,'Color','green','linestyle','--',"DisplayName","f(noise)="+notch_f2);
hold on
...
sig_f_noise1=notch_f1*fs2;
sig_f_noise2=notch_f2*fs2;
...
sig_n=A*cos(2*pi*sig_f*t)+A_noise_rate_1*A*cos(2*pi*sig_f_noise1*t)+...
      A_noise_rate_2*A*cos(2*pi*sig_f_noise2*t); 
